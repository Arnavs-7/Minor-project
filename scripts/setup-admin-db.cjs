...
async function main() {
  ...
  if (parseInt(existingCustomers[0].count) === 0) {
    ...
    const customers = [
      { name: "Admin", phone: "9999999999", email: "admin@msdiy.com", role: "admin" },
  ...
  if (parseInt(existingOrders[0].count) === 0) {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      await sql`
        INSERT INTO orders (customer_name, customer_phone, items, total, status, created_at)
        VALUES (${customerName}, ${"98765432" + (10 + (i % 7))}, ${JSON.stringify(items)}, ${total}, ${status}, ${date.toISOString()})
      `;
    }
