...
import { Search, Package, Pickaxe, Truck, CheckCircle, PackageOpen, X } from "lucide-react";
...
const fetchOrderDetails = async (id: string): Promise<any> => {
  const response = await fetch(`/api/order-status?id=${id}`);
  if (!response.ok) {
